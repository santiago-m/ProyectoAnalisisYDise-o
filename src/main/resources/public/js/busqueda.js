...
function tableHtml(usuario, nombre_partida, cPreguntas) {
  ...
  if (usuario.length > 0) {
    ...
    for (var i = usuario.length - 1; i >= 0; i--) {
      insert("partidas",('<tbody><tr><td>' + nombre_partida[i] + '</td><td>' + cPreguntas[i] + '</td><td>' + usuario[i] + '</td><td><center><form action="/selectHost" method="POST"><input name="hostName" value="'+ nombre_partida[i] +'" type="hidden"><input class="btn btn-outline-success" value="Unite!" type="submit"></form></td></tr></tbody>'));
